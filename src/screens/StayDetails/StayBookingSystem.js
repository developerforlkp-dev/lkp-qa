...
const StayBookingSystem = ({
  ...
  guests,
  setGuests,
  childAges: externalChildAges,
  setChildAges: externalSetChildAges,
  ...
  const childAges = useMemo(() => {
    if (Array.isArray(externalChildAges) && externalChildAges.length > 0) return externalChildAges;
    return internalChildAges;
  }, [externalChildAges, internalChildAges]);

  useEffect(() => {
    if (Array.isArray(externalChildAges) && externalChildAges.length > 0) {
      setInternalChildAges(externalChildAges);
    }
  }, [externalChildAges]);
  ...
  const capacityFeedback = useMemo(() => {
    ...
    if (isPropertyBased) {
      totalBaseAdultsLimit = Number(stay.maxAdults ?? stay.max_adults ?? stay.maxGuests ?? stay.occupancyPricing?.baseAdultCount ?? 1);
      totalBaseChildrenLimit = Number(stay.maxChildren ?? stay.max_children ?? stay.occupancyPricing?.baseChildCount ?? 0);
        ...
        try {
          ...
          if (freshAvailability) {
            ...
            if (isPropertyBased) {
              ...
              const baseAdultsMax = Number(stay.maxAdults ?? stay.max_adults ?? freshAvailability.occupancyPricing?.baseAdultCount ?? freshAvailability.occupancyPricing?.base_adult_count ?? freshAvailability.occupancyPricing?.maxAdultCount ?? freshAvailability.maxAdults ?? stay.maxGuests ?? 1);
              const extraAdultsAllowed = Number(stay.maxExtraAdultsAllowed ?? freshAvailability.occupancyPricing?.maxExtraAdultsAllowed ?? freshAvailability.maxExtraAdults ?? stay.maxExtraAdults ?? 0);
              ...
              const extraChildrenAllowed = Number(stay.maxExtraChildrenAllowed ?? freshAvailability.occupancyPricing?.maxExtraChildrenAllowed ?? freshAvailability.maxExtraChildren ?? stay.maxExtraChildren ?? 0);
