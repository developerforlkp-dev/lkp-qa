...
const getBookingScopeValue = (stay) => (
  String(
    ...
    stay?.booking_scope ||
    stay?.scope ||
    stay?.stayBookingScope ||
    stay?.stay_booking_scope ||
    stay?.stayType ||
    stay?.stay_type ||
    stay?.type ||
    ""
...
const isPropertyBasedBooking = (stay) => {
  if (
    stay?.fullPropertyB2cPrice != null ||
    stay?.fullPropertyb2cPrice != null ||
    stay?.full_property_b2c_price != null ||
    stay?.fullPropertyPrice != null ||
    stay?.fullPropertyExtraAdultPrice != null ||
    stay?.fullPropertyExtraChildPrice != null ||
    stay?.isPropertyBased === true ||
    stay?.is_property_based === true
  ) {
    return true;
  }
  const scope = getBookingScopeValue(stay);
  if (scope.includes("property")) return true;
  if (scope.includes("hotel") || scope.includes("room") || scope.includes("hostel")) return false;
  const rooms = stay?.rooms || stay?.stayRooms || stay?.stay_rooms || [];
...
const getChildAgePolicyTiers = (stay) => {
  ...
  // Support top-level stay child rate fields from stay/detail API
  ...
  if (Number.isFinite(childRateFrom) && Number.isFinite(childRateTo)) {
    const extraChildPrice = Number(
      stay?.fullPropertyExtraChildPrice ??
      stay?.extraChildPrice ??
      stay?.occupancyPricing?.extraChildPrice ??
      0
    );
    return [
      {
        fromAge: childRateFrom,
        toAge: childRateTo,
        price: Number.isFinite(extraChildPrice) ? extraChildPrice : 0,
      }
    ];
  }
...
const getComplimentaryChildAgeStart = (stay) => {
  const targetStay = stay?.stay || stay;
  const compStart = Number(
    targetStay?.complimentaryChildAgeStart ??
    targetStay?.complimentary_child_age_start ??
    targetStay?.complimentaryChildAgeFrom ??
    targetStay?.complimentary_child_age_from ??
    targetStay?.complimentaryAgeStart ??
    targetStay?.complimentary_age_start ??
    targetStay?.complimentaryAgeFrom ??
    targetStay?.complimentary_age_from ??
    targetStay?.childAgeComplimentaryStart ??
    targetStay?.child_age_complimentary_start ??
    targetStay?.childAgeComplimentaryFrom ??
    targetStay?.child_age_complimentary_from
  );
  if (Number.isFinite(compStart)) {
    return compStart;
  }
  return 0;
};

const getComplimentaryChildAgeEnd = (stay) => {
  const targetStay = stay?.stay || stay;
  const compEnd = Number(
    targetStay?.complimentaryChildAgeEnd ??
    targetStay?.complimentary_child_age_end ??
    targetStay?.complimentaryChildAgeTo ??
    targetStay?.complimentary_child_age_to ??
    targetStay?.complimentaryAgeEnd ??
    targetStay?.complimentary_age_end ??
    targetStay?.complimentaryAgeTo ??
    targetStay?.complimentary_age_to ??
    targetStay?.childAgeComplimentaryEnd ??
    targetStay?.child_age_complimentary_end ??
    targetStay?.childAgeComplimentaryTo ??
    targetStay?.child_age_complimentary_to
  );
  if (Number.isFinite(compEnd)) {
    return compEnd;
  }
  return 5;
};
...
const getChildAgePolicyBounds = (stay) => {
  const targetStay = stay?.stay || stay;
  const tiers = getChildAgePolicyTiers(targetStay);
  const compFrom = getComplimentaryChildAgeStart(targetStay);
  const compTo = getComplimentaryChildAgeEnd(targetStay);
  const rateFrom = Number(
    targetStay?.childAgeRateFrom ??
    targetStay?.child_age_rate_from ??
    targetStay?.childRateAgeFrom ??
    targetStay?.child_rate_age_from
  );
  const rateTo = Number(
    targetStay?.childAgeRateTo ??
    targetStay?.child_age_rate_to ??
    targetStay?.childRateAgeTo ??
    targetStay?.child_rate_age_to ??
    targetStay?.childRateTo ??
    targetStay?.child_rate_to
  );
  ...
  return {
    minAge: Math.min(compFrom, Number.isFinite(rateFrom) ? rateFrom : compFrom),
    maxAge: Math.max(minAge, maxAge),
    complimentaryFrom: compFrom,
    complimentaryTo: compTo,
    childRateFrom: Number.isFinite(rateFrom) ? rateFrom : minAge,
    childRateTo: Number.isFinite(rateTo) ? rateTo : maxAge,
  };
};

const getSelectableChildAges = (stay) => {
  const targetStay = stay?.stay || stay;
  const defaultAge = getComplimentaryChildAgeStart(targetStay);
  ...
  const bounds = getChildAgePolicyBounds(targetStay);
  const min = 0;
  let max;
...
const StayBookingSystem = ({
  ...
  const onToggleAddOn = (addon) => {
    setSelectedAddOns(prev => {
      const addonId = String(addon?.addonId || addon?.id || addon);
      const exists = prev.some(a => String(a?.addonId || a?.id || a) === addonId);
      if (exists) return prev.filter(a => String(a?.addonId || a?.id || a) !== addonId);
      return [...prev, addon];
    });
  };

  const onAddOnQuantityChange = (addonId, qty, addon) => {
    const aid = String(addonId);
    if (qty <= 0) {
      setSelectedAddOns(prev => prev.filter(a => String(a?.addonId || a?.id || a) !== aid));
      ...
      setAddOnQuantities(prev => ({ ...prev, [aid]: qty }));
      setSelectedAddOns(prev => {
        if (prev.some(a => String(a?.addonId || a?.id || a) === aid)) return prev;
        return [...prev, addon || addonId];
      });
    }
  };
  // ---------------------------------
      ...
      if (storedRaw) {
        ...
        if (stored?.listingId === String(stay?.stayId || stay?.id) && stored?.type === "stay" && isLoggedIn) {
          ...
          if (Array.isArray(stored.childAges)) {
            setChildAges(syncChildAges(stored.childAges, stored?.guests?.children || 0, defaultChildAge));
          }
  ...
  const resolvedSelectedRooms = useMemo(() => {
    ...
    const rawRoomsSource = (
      ...
      availabilityData?.data?.roomAvailability?.rooms ||
      availabilityData?.data?.rooms ||
      availabilityData?.data?.roomAvailability ||
      (Array.isArray(availabilityData?.data) ? availabilityData.data : []) ||
  ...
  useEffect(() => {
    ...
    setChildAges((prev) => syncChildAges(prev, guests?.children || 0, defaultChildAge));
  }, [guests?.children, setChildAges, defaultChildAge]);

  useEffect(() => {
    if (show && guests?.children > 0 && typeof setChildAges === "function") {
      setChildAges((prev) => {
        const safeCount = Math.max(0, Number(guests?.children || 0));
        const arr = Array.isArray(prev) ? [...prev] : [];
        let changed = false;
        while (arr.length < safeCount) {
          arr.push(defaultChildAge);
          changed = true;
        }
        for (let i = 0; i < safeCount; i++) {
          if (arr[i] === "" || arr[i] === null || arr[i] === undefined) {
            arr[i] = defaultChildAge;
            changed = true;
          }
        }
  ...
  }, [show, guests?.children, defaultChildAge, setChildAges]);
  ...
  const pricing = useMemo(() => {
    ...
    if (isPropertyBased) {
      totalBaseAdultsLimit = Number(stay.maxAdults ?? stay.max_adults ?? stay.maxGuests ?? stay.occupancyPricing?.baseAdultCount ?? stay.occupancyPricing?.base_adult_count ?? 1);
      totalBaseChildrenLimit = Number(stay.maxChildren ?? stay.max_children ?? stay.occupancyPricing?.baseChildCount ?? stay.occupancyPricing?.base_child_count ?? 0);
      totalExtraAdultsLimit = Number(stay.maxExtraAdultsAllowed ?? stay.maxExtraAdults ?? stay.occupancyPricing?.maxExtraAdultsAllowed ?? stay.occupancyPricing?.max_extra_adults_allowed ?? stay.maxExtraBeds ?? 0);
      totalExtraChildrenLimit = Number(stay.maxExtraChildrenAllowed ?? stay.maxExtraChildren ?? stay.occupancyPricing?.maxExtraChildrenAllowed ?? stay.occupancyPricing?.max_extra_children_allowed ?? 0);
    ...
    return {
      ...
      childAges: syncChildAges(childAges, guests?.children || 0, defaultChildAge),
      childAgePricing,
  ...
  }, [stay, resolvedSelectedRooms, checkInDate, guests, childAges, nightsCount, selectedRooms, stayRoomsCatalog, selectedAddOns, addOnQuantities, defaultChildAge]);
  ...
  useEffect(() => {
    ...
    const distribution = distributeGuests(selectedRooms, stayRoomsCatalog, guests.adults || 1, guests.children || 0);
    const normalizedChildAges = syncChildAges(childAges, guests.children || 0, defaultChildAge).map((age) => {
      if (age !== "" && age !== null && age !== undefined && !Number.isNaN(Number(age))) {
        return Number(age);
      }
    ...
    const grouped = {};
    if (distribution.success || (distribution.allocations && distribution.allocations.length > 0)) {
      distribution.allocations.forEach(alloc => {
    ...
    let requiredExtraAdultCount = 0;
    let requiredExtraChildCount = 0;
    let propChildAges = [];
    ...
    if (isPropertyBased) {
      ...
      const propBaseAdults = Number(
        stay?.maxAdults ??
        stay?.max_adults ??
        stay?.maxGuests ??
        stay?.occupancyPricing?.baseAdultCount ??
        stay?.occupancyPricing?.base_adult_count ??
        stay?.occupancyPricing?.maxAdultCount ??
        1
      );
      const propBaseChild = Number(
        stay?.maxChildren ??
        stay?.max_children ??
        stay?.occupancyPricing?.baseChildCount ??
        stay?.occupancyPricing?.base_child_count ??
        stay?.baseChildCount ??
        stay?.base_child_count ??
        0
      );
  ...
  const extraChildAgeIndexes = useMemo(() => {
    ...
    const propExtraChildren = Number(
      stay?.maxExtraChildrenAllowed ??
      stay?.maxExtraChildren ??
      stay?.occupancyPricing?.maxExtraChildrenAllowed ??
      stay?.occupancyPricing?.max_extra_children_allowed ??
      0
    );
    const baseChildrenLimit = isPropertyBased
      ? propBaseChildren
      : (Number(pricing.baseChildrenLimit || 0) > 0 ? Number(pricing.baseChildrenLimit) : propBaseChildren);

    const extraLimit = isPropertyBased
      ? propExtraChildren
      : Number(pricing.extraChildrenLimit || 0);

    const currentChildren = Number(guests?.children || 0);
    if (currentChildren <= baseChildrenLimit) return [];

    const rawExtra = currentChildren - baseChildrenLimit;
    const count = extraLimit > 0 ? Math.min(rawExtra, extraLimit) : rawExtra;
    return Array.from({ length: count }, (_, index) => baseChildrenLimit + index);
  }, [stay, guests?.children, pricing.baseChildrenLimit, pricing.extraChildrenLimit]);
  ...
  const capacityFeedback = useMemo(() => {
    ...
    if (isPropertyBased) {
      totalBaseAdultsLimit = Number(stay.maxAdults ?? stay.max_adults ?? stay.maxGuests ?? stay.occupancyPricing?.baseAdultCount ?? stay.occupancyPricing?.base_adult_count ?? stay.occupancyPricing?.maxAdultCount ?? 1);
      totalBaseChildrenLimit = Number(stay.maxChildren ?? stay.max_children ?? stay.occupancyPricing?.baseChildCount ?? stay.occupancyPricing?.base_child_count ?? stay.baseChildCount ?? stay.base_child_count ?? 0);
      totalExtraAdultsLimit = Number(stay.maxExtraAdultsAllowed ?? stay.maxExtraAdults ?? stay.occupancyPricing?.maxExtraAdultsAllowed ?? stay.maxExtraBeds ?? 0);
      totalExtraChildrenLimit = Number(stay.maxExtraChildrenAllowed ?? stay.maxExtraChildren ?? stay.occupancyPricing?.maxExtraChildrenAllowed ?? 0);
    } else {
      resolvedSelectedRooms.forEach(room => {
        totalBaseAdultsLimit += (room.maxAdults || 1) * room.count;
        totalBaseChildrenLimit += (room.maxChildren || 0) * room.count;
    ...
    if (guests.adults > allowedAdults) {
      return {
        text: isPropertyBased ? `Adult limit reached (max ${allowedAdults} adults allowed).` : "Adult limit reached even with extra adults. Add another room.",
        type: "error",
        exceeded: true
      };
    }
    ...
    if (guests.children > allowedChildren) {
      return {
        text: isPropertyBased ? `Children limit reached (max ${allowedChildren} children allowed).` : "Children limit reached even with extra guests. Add another room.",
        type: "error",
        exceeded: true
      };
    }
  ...
  const handleReserve = async () => {
    ...
    try {
      const isPropertyBased = isPropertyBasedBooking(stay);
      const isHostel = isHostelBooking(stay);
      const stayIdVal = stay?.stayId || stay?.id;
      const checkInStr = checkInDate.format("YYYY-MM-DD");
      const checkOutStr = checkOutDate.format("YYYY-MM-DD");

      // Verify real-time property, room, or bed availability when clicking Reserve button in modal
      if (stayIdVal && checkInStr && checkOutStr) {
        try {
          const freshRaw = isPropertyBased
            ? await getStayPropertyAvailability(stayIdVal, checkInStr, checkOutStr)
            : (isHostel
                ? await getStayHostelAvailability(stayIdVal, checkInStr, checkOutStr)
                : await getStayHotelRoomAvailability(stayIdVal, checkInStr, checkOutStr));
          ...
          if (freshAvailability) {
            setAvailabilityData(freshAvailability);
            ...
            if (!isOverallAvailable || freshAvailability.sameDayCheckInClosed === true) {
              ...
              setBookingErrorPopup({
                visible: true,
                title: freshAvailability.sameDayCheckInClosed ? "Check-in Closed" : "Stay Unavailable",
                message: reason,
                isSameDay: Boolean(freshAvailability.sameDayCheckInClosed),
              });
              return;
            }
            ...
            if (isPropertyBased) {
              if (Number(freshAvailability.overlappingBookings || 0) > 0 || freshAvailability.isBooked === true) {
                setLoading(false);
                const reason = "This property is already booked for the selected dates.";
                setValidationError(reason);
                setBookingErrorPopup({
                  visible: true,
              ...
              const baseAdultsMax = Number(stay.maxAdults ?? stay.max_adults ?? stay.maxGuests ?? freshAvailability.occupancyPricing?.baseAdultCount ?? freshAvailability.occupancyPricing?.base_adult_count ?? freshAvailability.occupancyPricing?.maxAdultCount ?? freshAvailability.maxAdults ?? 1);
              const extraAdultsAllowed = Number(stay.maxExtraAdultsAllowed ?? stay.maxExtraAdults ?? freshAvailability.occupancyPricing?.maxExtraAdultsAllowed ?? freshAvailability.maxExtraAdults ?? stay.maxExtraBeds ?? 0);
              const totalAllowedAdults = baseAdultsMax + extraAdultsAllowed;

              const baseChildrenMax = Number(stay.maxChildren ?? stay.max_children ?? freshAvailability.occupancyPricing?.baseChildCount ?? freshAvailability.occupancyPricing?.base_child_count ?? freshAvailability.maxChildren ?? 0);
              const extraChildrenAllowed = Number(stay.maxExtraChildrenAllowed ?? stay.maxExtraChildren ?? freshAvailability.occupancyPricing?.maxExtraChildrenAllowed ?? freshAvailability.maxExtraChildren ?? 0);
              ...
              if (hasBedSelection && freshAvailability.canBookBedOnly === false && !hasRoomSelection) {
                setLoading(false);
                const reason = "Bed booking is currently unavailable for this hostel.";
                setValidationError(reason);
                setBookingErrorPopup({ visible: true, title: "Beds Unavailable", message: reason, isSameDay: false });
                return;
              }
              if (hasRoomSelection && freshAvailability.canBookRoomOnly === false && !hasBedSelection) {
                setLoading(false);
              ...
              if (hasBedSelection) {
                const bedAvail = freshAvailability.bedAvailability || {};
                if (bedAvail.isAvailable === false) {
                  setLoading(false);
                  const reason = "Beds are unavailable for the selected dates.";
                  setValidationError(reason);
                  setBookingErrorPopup({ visible: true, title: "Beds Unavailable", message: reason, isSameDay: false });
                  return;
                }
                const totalBedsRequired = totalSelectedBeds > 0 ? totalSelectedBeds : (guests.adults || 1);
                const availableBedsCount = Number(
                  bedAvail.availableBeds ??
                  freshAvailability.availableBeds ??
                  freshAvailability.remainingBeds ??
                  freshAvailability.availableUnits ??
                  freshAvailability.units
                );
                if (Number.isFinite(availableBedsCount) && availableBedsCount < totalBedsRequired) {
                  setLoading(false);
                  const reason = availableBedsCount === 0
                    ? "No beds are available for the selected dates."
                    : `Only ${availableBedsCount} bed(s) available for the selected dates (you requested ${totalBedsRequired}).`;
                  setValidationError(reason);
                  setBookingErrorPopup({
                    visible: true,
              ...
              // Check hostel private rooms
              if (hasRoomSelection) {
                const hostelRooms = freshAvailability.roomAvailability?.rooms ||
                  freshAvailability.rooms ||
                  freshAvailability.roomAvailability ||
                  [];
                for (const sel of nonBedRoomsSelected) {
                  const selKey = String(sel.roomId ?? sel.id ?? sel.roomTypeId ?? sel.room_type_id ?? sel.bedConfigId ?? "").replace(/^bed-/, "").trim().toLowerCase();
                  const selName = String(sel.roomName || sel.name || sel.roomTypeName || "").trim().toLowerCase();

                  const matched = Array.isArray(hostelRooms) ? hostelRooms.find((r) => {
                  ...
                  const reqCount = Number(sel.count || 1);
                  const roomLabel = matched?.roomName || matched?.name || sel.roomName || sel.name || "Selected Room";

                  if (matched) {
                    const isRoomAvail = !(matched.isAvailable === false || matched.is_available === false || matched.available === false || matched.status === "UNAVAILABLE" || matched.isBooked === true);
                    const availCount = Number(matched.availableRooms ?? matched.available_rooms ?? matched.availableUnits ?? matched.units);
                    if (!isRoomAvail || (Number.isFinite(availCount) && availCount < reqCount)) {
                      setLoading(false);
                      const reason = (!isRoomAvail || availCount === 0)
                        ? `Room "${roomLabel}" is not available for the selected dates.`
                        : `Room "${roomLabel}" only has ${availCount} room(s) available (you selected ${reqCount}).`;
                      setValidationError(reason);
                      setBookingErrorPopup({ visible: true, title: "Room Unavailable", message: reason, isSameDay: false });
                      return;
                    }
                  } else if (Array.isArray(hostelRooms) && hostelRooms.length > 0) {
                    setLoading(false);
                    const reason = `Room "${roomLabel}" is not available for the selected dates.`;
                    setValidationError(reason);
                    setBookingErrorPopup({ visible: true, title: "Room Unavailable", message: reason, isSameDay: false });
                    return;
                  }
              ...
              // Hotel / Room-based validation
              if (freshAvailability.hasAvailableRooms === false) {
                setLoading(false);
                const reason = freshAvailability.availabilityReason || "No rooms are available for the selected dates.";
                setValidationError(reason);
                setBookingErrorPopup({ visible: true, title: "No Rooms Available", message: reason, isSameDay: false });
                return;
              }
                ...
                for (const sel of selectedRooms) {
                  ...
                  const matched = (Array.isArray(availRooms) && availRooms.length > 0)
                    ? availRooms.find((r) => {
                        ...
                        const rName = String(r.roomName || r.name || r.roomTypeName || "").trim().toLowerCase();
                        if (selName && rName && selName === rName) return true;
                  ...
                  if (matched) {
                    ...
                    if (!isRoomAvailable || (availableCount !== undefined && availableCount < reqCount)) {
                      setLoading(false);
                      const reason = (!isRoomAvailable || availableCount === 0)
                        ? `Room "${roomLabel}" is not available for the selected dates.`
                        : `Room "${roomLabel}" only has ${availableCount} room(s) available (you selected ${reqCount}).`;
                      setValidationError(reason);
                      setBookingErrorPopup({
                        visible: true,
          ...
          console.warn("⚠️ Error checking availability during reserve:", availErr);
          ...
          if (status === 400 || status === 409 || status === 422 || (errMsg && (errMsg.toLowerCase().includes("avail") || errMsg.toLowerCase().includes("book") || errMsg.toLowerCase().includes("room")))) {
            setLoading(false);
            const reason = errMsg || "The selected rooms or dates are unavailable. Please choose different dates.";
            setValidationError(reason);
            setBookingErrorPopup({
              visible: true,
      ...
      if (canUseChildAgeSelector) {
        const requiredExtraChildAgeIndexes = extraChildAgeIndexes;
      ...
      const backendAddOns = Array.isArray(selectedAddOns)
        ...
        : [];
      const normalizedChildAges = syncChildAges(childAges, guests.children || 0, defaultChildAge)
        .map((age) => (age !== "" && age !== null && age !== undefined && !Number.isNaN(Number(age))) ? Number(age) : defaultChildAge);
      ...
      if (isPropertyBased) {
        ...
        const requiredExtraChildCount = Math.max(0, Number(guests.children || 0) - propBaseChild);
        const propChildAges = Array.from({ length: requiredExtraChildCount }, (_, i) => {
          const ageVal = childAges?.[propBaseChild + i];
          if (ageVal !== "" && ageVal !== null && ageVal !== undefined && !Number.isNaN(Number(ageVal))) {
            return Number(ageVal);
          }
          return Number(selectableChildAges?.[0] ?? 0);
        });
                  ...
                  <div className="booking-modal-column" style={{ padding: "20px 28px", background: BG, display: "flex", flexDirection: "column", gap: 16 }}>
                    ...
                    <div id="booking-guests-section" style={{ scrollMarginTop: "24px" }}>
                      ...
                      {(() => {
                        ...
                        const propBaseAdults = Number(stay?.maxAdults ?? stay?.max_adults ?? stay?.maxGuests ?? 1);
                        const propExtraAdults = Number(stay?.maxExtraAdultsAllowed ?? stay?.maxExtraAdults ?? stay?.occupancyPricing?.maxExtraAdultsAllowed ?? 0);
                        const propMaxAdults = propBaseAdults + propExtraAdults;

                        const propBaseChildren = Number(stay?.maxChildren ?? stay?.max_children ?? 0);
                        const propExtraChildren = Number(stay?.maxExtraChildrenAllowed ?? stay?.maxExtraChildren ?? stay?.occupancyPricing?.maxExtraChildrenAllowed ?? 0);
                        const propMaxChildren = propBaseChildren + propExtraChildren;

                        let absoluteMaxAdults = isPropertyBased ? propMaxAdults : allowedAdults;
                        let absoluteMaxChildren = isPropertyBased ? propMaxChildren : allowedChildren;
                        ...
                        return (
                          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                            ...
                            {/* Child Age Selector: show when extra children exist (property-based or room-based) */}
                            {extraChildAgeIndexes.length > 0 && (() => {
                              const _extraIndexes = extraChildAgeIndexes;
                              const compFrom = extraChildPolicyBounds?.complimentaryFrom ?? getComplimentaryChildAgeStart(stay);
                              const compTo = extraChildPolicyBounds?.complimentaryTo ?? getComplimentaryChildAgeEnd(stay);
                              const rateFrom = extraChildPolicyBounds?.childRateFrom ?? Number(stay?.childAgeRateFrom ?? stay?.child_age_rate_from ?? stay?.childRateAgeFrom ?? stay?.child_rate_age_from ?? 6);
                              const rateTo = extraChildPolicyBounds?.childRateTo ?? Number(stay?.childAgeRateTo ?? stay?.child_age_rate_to ?? stay?.childRateAgeTo ?? stay?.child_rate_age_to ?? stay?.childRateTo ?? stay?.child_rate_to ?? 12);
                              const extraChildPrice = pricing?.activeExtraChildPrice || Number(stay?.fullPropertyExtraChildPrice || stay?.extraChildPrice || 0);
                              return (
                                <div style={{ marginTop: 6, padding: "16px", background: BG, border: `1px solid ${B}`, borderRadius: 16, display: "flex", flexDirection: "column", gap: 12 }}>
                                  <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
                                    ...
                                    <div style={{ display: "flex", flexDirection: "column", gap: 6, flex: 1 }}>
                                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 4 }}>
                                        <span style={{ fontSize: 13, fontWeight: 700, color: FG }}>Extra Child Age Policy</span>
                                      </div>
                                      <span style={{ fontSize: 11, fontWeight: 400, color: M, lineHeight: 1.35 }}>
                                  ...
                                  <div style={{ display: "grid", gridTemplateColumns: _extraIndexes.length === 1 ? "1fr" : "1fr 1fr", gap: 8, marginTop: 4 }}>
                                    {_extraIndexes.map((childIndex) => {
                                      const currentAge = childAges?.[childIndex] !== "" && childAges?.[childIndex] != null ? Number(childAges[childIndex]) : defaultChildAge;
                                      const isComp = currentAge >= compFrom && currentAge <= compTo;
                                      const isRate = currentAge >= rateFrom && currentAge <= rateTo;
                                      return (
                                        <div key={`extra-child-age-${childIndex}`} style={{ display: "flex", flexDirection: "column", minWidth: 0 }}>
                                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 8px", gap: 4, background: `${B}22`, border: `1px solid ${B}66`, borderRadius: 12 }}>
                                            <div style={{ display: "flex", alignItems: "center", gap: 4, minWidth: 0, flexShrink: 1 }}>
                                              <div style={{ width: 6, height: 6, borderRadius: "50%", background: isComp ? "#16a34a" : A, flexShrink: 0 }}></div>
                        ...
                        <div style={{
                          ...
                          background: capacityFeedback.type === "success"
                            ...
                            : capacityFeedback.type === "warning" || capacityFeedback.type === "error"
                              ? `${E}0c`
                              : `${M}0c`,
                          border: `1px solid ${capacityFeedback.type === "success"
                              ? `${A}22`
                              : capacityFeedback.type === "warning" || capacityFeedback.type === "error"
                                ? `${E}22`
                                : `${B}`
